...
// `student-quiz-attempt-http.postgres-test.ts` already established for this codebase.
const NOW = new Date('2026-06-15T12:00:00.000Z');
const ONE_DAY_MS = 24 * 60 * 60 * 1000;

maybeDescribe('student document access HTTP PostgreSQL integration', () => {
  ...
  async function setUpAccessibleDocumentLesson(
    slugSuffix: string,
    options: { processingStatus?: AssetProcessingStatus } = {},
  ): Promise<Setup> {
    ...
    const documentAssetId = await createDocumentAssetDirect(
      tenantId,
      instructorId,
  ...
  function accessPath(courseId: string, lessonId: string): string {
    return `/student/courses/${courseId}/lessons/${lessonId}/document/access`;
  }
  ...
  it('creates no LessonProgress, QuizAttempt, or other side-effect row on authorization', async () => {
    const setup = await setUpAccessibleDocumentLesson('no-side-effects');
  ...
  it('denies a random Lesson ID and a random Course ID with a non-leaking not-found response', async () => {
    const setup = await setUpAccessibleDocumentLesson('random-ids');
  ...
  it('denies a foreign Course/Lesson pair (own Course, another Course-tenant Lesson ID)', async () => {
    const setup = await setUpAccessibleDocumentLesson('foreign-pair-a');
    const other = await setUpAccessibleDocumentLesson('foreign-pair-b');
  ...
  it('denies a DOCUMENT Lesson belonging to another Tenant even with a valid Enrollment/device for the caller tenant', async () => {
    const setup = await setUpAccessibleDocumentLesson('cross-tenant-a');
    const other = await setUpAccessibleDocumentLesson('cross-tenant-b');
  ...
  it('denies substituting another student\'s valid Enrollment/Course/Lesson for the caller', async () => {
    const owner = await setUpAccessibleDocumentLesson('substitution-owner');
    const attacker = await setUpAccessibleDocumentLesson('substitution-attacker');
  ...
  it('cannot leak a cross-tenant DocumentAsset relationship: the schema rejects attaching a foreign-tenant asset to a Lesson', async () => {
    const setup = await setUpAccessibleDocumentLesson('cross-tenant-fk-a');
    const other = await setUpAccessibleDocumentLesson('cross-tenant-fk-b');

    // Proves at the data-integrity level (not just the HTTP boundary) that a DocumentLesson can
    // never reference a DocumentAsset outside its own tenant: the composite foreign key
    // `DocumentLesson(documentAssetId, tenantId) -> DocumentAsset(id, tenantId)` makes this an
    // unreachable database state, which is exactly why `assertAccessibleDocumentLesson` can trust
    // a resolved `documentAssetId` as already tenant-proven with no separate check.
    ...
    await expect(
      prisma.client.documentLesson.create({
        data: { lessonId: foreignLessonId, tenantId: setup.tenantId, documentAssetId: other.documentAssetId },
      }),
    ).rejects.toThrow();
  });
  ...
  it('denies access with no approved device', async () => {
    const setup = await setUpAccessibleDocumentLesson('no-device');
  ...
  it('denies access to a Lesson under an unpublished Section', async () => {
    for (const status of [SectionStatus.DRAFT, SectionStatus.ARCHIVED]) {
      const setup = await setUpAccessibleDocumentLesson(`section-status-${status}`);
      // Overwrite the already-created section's status directly rather than the read entitlement
      // API, mirroring the pattern used for other lifecycle-boundary tests in this codebase.
  ...
  it('denies access to an unpublished Lesson', async () => {
    for (const status of [LessonStatus.DRAFT, LessonStatus.ARCHIVED]) {
      const setup = await setUpAccessibleDocumentLesson(`lesson-status-${status}`);
  ...
  it('denies access before availableFrom', async () => {
    const setup = await setUpAccessibleDocumentLesson('available-from');
  ...
  it('denies access at and after availableUntil', async () => {
    const setup = await setUpAccessibleDocumentLesson('available-until');
    await prisma.client.lesson.update({
      where: { id: setup.lessonId },
  ...
  it('denies access to a VIDEO Lesson via the document access route', async () => {
    const setup = await setUpAccessibleDocumentLesson('video-lesson');
    ...
    const response = await getAccess(setup, setup.courseId, videoLessonId).expect(HttpStatus.NOT_FOUND);
    expect(response.body).toMatchObject({ error: { code: 'LESSON_NOT_FOUND' } });
  });

  it('denies access to a QUIZ Lesson via the document access route', async () => {
    const setup = await setUpAccessibleDocumentLesson('quiz-lesson');
  ...
  it('denies access to a non-READY DocumentAsset (UPLOADING, PROCESSING, FAILED, ARCHIVED)', async () => {
    for (const processingStatus of [
      ...
      const setup = await setUpAccessibleDocumentLesson(`not-ready-${processingStatus}`, { processingStatus });
  ...
  it('authorizes access once the DocumentAsset transitions to READY', async () => {
    const setup = await setUpAccessibleDocumentLesson('becomes-ready', {
  ...
  async function clearData(): Promise<void> {
    ...
    await prisma.client.tenant.deleteMany({
      where: { slug: { startsWith: 'student-document-test-' } },
    });
  ...
  async function createInstructorTenant(slugSuffix: string): Promise<{ instructorId: string; tenantId: string }> {
    ...
    const tenant = await prisma.client.tenant.create({
      data: {
        id: uuid.create(),
        name: `Student Document Tenant ${slugSuffix}`,
        slug: `student-document-test-${slugSuffix}`,
