...
## DEC-0048: Explicit Authoring Lifecycle Transitions

- Date: 2026-08-31
- Status: Accepted
- Decision: Course, CourseSection, Lesson, and Quiz publication/archive state changes use dedicated instructor lifecycle endpoints and service methods, not generic metadata updates accepting arbitrary `status` values. A `PUBLISHED` Quiz's aggregate publishability (the same rule `publishQuiz()` itself enforces) is additionally re-validated, transactionally, after every subsequent Question/Option authoring mutation capable of affecting it.
- Reasoning: Student authorization already depends on `PUBLISHED` gates, so publication must be server-controlled, tenant-authorized, race-safe, and able to enforce resource-specific prerequisites. A `PUBLISHED` Quiz that authoring edits are still allowed to reach an unpublishable aggregate state (e.g. a Question left with zero correct Options) would silently break that same guarantee for students already relying on it, so ordinary Quiz authoring mutations need the identical safeguard publication itself has, not just a one-time check at the DRAFT -> PUBLISHED transition.
- Implications: Supported V1 transitions are `DRAFT -> PUBLISHED`, `DRAFT -> ARCHIVED`, `PUBLISHED -> ARCHIVED`, `PUBLISHED -> DRAFT` via explicit Take Offline, and `ARCHIVED -> DRAFT` via explicit Restore. Publish/archive/restore actions are idempotent for already-target-state resources, do not cascade to ancestors or descendants, and ordinary authoring edits remain allowed for non-archived resources under the existing snapshot/readiness safeguards. A `DRAFT` Quiz is exempt from the aggregate check and may stay incomplete indefinitely while authored; a `PUBLISHED` Quiz is not - a Question/Option mutation that would leave it unpublishable is rejected atomically, leaving the prior valid state unchanged. Creating a new Question is rejected outright while its Quiz is `PUBLISHED`, since this API always creates a Question before any Option, so a brand-new Question can never itself satisfy "exactly one correct option." `publishQuiz()`, `unpublishQuiz()`, `restoreQuiz()`, `archiveQuiz()`, and every publishability-affecting mutation share one PostgreSQL transaction-scoped advisory lock keyed on the Quiz ID (Option mutations acquire it before their existing Question-scoped lock, never after, keeping lock ordering deadlock-free), so a concurrent publish can never race a concurrent mutation into leaving a `PUBLISHED` Quiz aggregate-invalid. No schema or migration change was required.

### 2026-09-03 Addendum: Explicit Take Offline

- Decision: Course, CourseSection, Lesson, and Quiz now also support explicit instructor take-offline endpoints for `PUBLISHED -> DRAFT`.
- Reasoning: Instructors need a reversible way to make live authored content non-live without archiving it, deleting it, or disturbing historical student data.
- Implications: Take Offline is non-cascading, preserves existing child lifecycle states, ordering, content references, enrollments, progress, quiz attempts, quiz attempt answers, and existing `publishedAt` values on Course and Quiz. Existing publish endpoints make taken-offline resources live again. `ARCHIVED` cannot be revived through unpublish and requires the separate explicit Restore action. Quiz unpublish uses the same Quiz-level publication-boundary advisory lock as publish/archive and publishability-affecting Question/Option mutations; Course, Section, and Lesson continue to rely on their existing transactional conditional-update/re-check lifecycle pattern, with no additional advisory locks.

### 2026-09-03 Addendum: Explicit Restore

- Decision: Course, CourseSection, Lesson, and Quiz now support explicit instructor restore endpoints for `ARCHIVED -> DRAFT`.
- Reasoning: Archive is recoverable authored-content lifecycle state, but recovery must be deliberate, non-live by default, and must not infer the resource's prior status or disturb historical student data.
- Implications: Restore is non-cascading, preserves child lifecycle states, ordering, content references, availability metadata, enrollments, progress, quiz attempts, quiz attempt answers, Question statuses, Options, and existing `publishedAt` values on Course and Quiz. Restored resources remain Draft until existing publish endpoints are called and pass their current validations. Quiz restore uses the same Quiz-level publication-boundary advisory lock as publish/archive/unpublish and publishability-affecting Question/Option mutations; Course, Section, and Lesson continue to rely on the transactional conditional-update/re-check lifecycle pattern, with no additional advisory locks. Question restore, media restore, student lifecycle restore, readiness, and publish-selected are not part of this slice. No schema or migration change was required.

## DEC-0049: Server-Side Course Readiness Derivation

- Date: 2026-09-03
- Status: Accepted
- Decision: `GET /instructor/tenants/:tenantId/courses/:courseId/readiness` derives Course publish readiness entirely server-side, from the exact Section/Lesson/VideoAsset/DocumentAsset/Quiz records the Course's own Lesson relations reference, and returns stable machine-readable `reasonCode`s (never translated/localized copy) plus a `readyToPublish` candidate set for the future `POST .../courses/:courseId/publish-selected`. Lifecycle state and content readiness are distinct concepts: `DRAFT` is the expected, normal state of a first-publish candidate, never a blocker by itself (there is no "Section/Lesson is DRAFT" reason code). A Lesson is a candidate iff `status === DRAFT`, its Section is not `ARCHIVED`, and its content is currently publishable; an already-`PUBLISHED` Lesson is never a candidate (already live, nothing to transition) regardless of its Section's status. A Section is a candidate iff `status === DRAFT` AND it contains at least one candidate Lesson — an already-`PUBLISHED` Section, or an empty/all-unready `DRAFT` Section, is never a candidate. A Quiz is listed in `readyToPublish.quizzes` only when it is still `DRAFT` (the future endpoint will need to transition it); an already-`PUBLISHED` Quiz backing a candidate Lesson is not listed. `ready` means "a valid first-publish selection exists" — true iff `readyToPublish.lessons` is non-empty (actual student-consumable content, not merely a technically-legal empty Section) — not "every descendant is ready": unfinished Draft content elsewhere is reported as blockers/simply absent from candidates without flipping the whole Course to not-ready. QUIZ Lesson content-readiness reuses the exact Quiz-aggregate publishability rule `QuizService.publishQuiz()` already enforces (factored into a pure `evaluateQuizPublishability` evaluator so existing throw-based behavior is unchanged), evaluated against the Quiz's current aggregate regardless of DRAFT/PUBLISHED status — intentionally broader than the existing standalone Lesson-publish gate, which still requires the Quiz to already be PUBLISHED.
- Reasoning: The prior Instructor Web client-side readiness derivation resolved Lesson asset state against one paginated page of the tenant's Media list (`MEDIA_PAGE_SIZE = 20`); a referenced asset outside that page silently resolved to unknown/not-ready regardless of its true state. Deriving readiness from the Course's own bounded relational structure instead of any tenant-wide paginated list makes that class of bug structurally impossible. An initial version of this endpoint required Sections/Lessons to already be individually `PUBLISHED` to count as candidates, which would have forced the instructor back into the exact multi-click "publish every Section, then every Lesson" workflow this endpoint exists to remove for a Course's first publish; the candidacy rule was corrected before this was approved, to align with the approved `publish-selected` contract of selecting currently-`DRAFT` content to transition.
- Implications: One bounded, nested Prisma read per request, scaled to the Course's own structure — no per-lesson or per-asset query, no tenant-wide Media/Quiz list consulted. Read-only: no PostgreSQL advisory lock is acquired, since nothing is mutated. A Lesson whose declared `type` has no matching type-detail row (provably impossible through `LessonService.createLesson`'s atomic creation) surfaces as a new `COURSE_DATA_INTEGRITY_VIOLATION` (500) rather than a silently false-Ready response. `VIDEO_ASSET_ARCHIVED`/`DOCUMENT_ASSET_ARCHIVED` are represented through the existing `AssetProcessingStatus.ARCHIVED` enum value already in the schema; no new media field, lifecycle state, or migration was added. Content-issue blockers (video/document/quiz) are still evaluated for every non-ARCHIVED Lesson regardless of its own DRAFT/PUBLISHED status, so the endpoint remains useful for diagnosing already-published Course content too. `readyToPublish` is informational only — the future publish-selected endpoint will never trust a client-supplied ID from it and will re-resolve every reference server-side. No schema or migration change was required.

## DEC-0050: Server-Side First-Publish Selection (publish-selected)

- Date: 2026-09-03
- Status: Accepted
- Decision: `POST /instructor/tenants/:tenantId/courses/:courseId/publish-selected` publishes exactly the Instructor's explicitly-reviewed selection (`sectionIds`/`lessonIds`, no quizId/asset ID/readiness flag/"exclude" list) for a Course's *first* publication only (`Course.publishedAt === null`; an already-published-before Course — live now, or Draft again after Take Offline — rejects with `COURSE_ALREADY_PUBLISHED_ONCE` and must use the existing granular `/publish` instead, which this slice leaves byte-for-byte unchanged). A selected Lesson's Section must either also be selected or already be `PUBLISHED` (the server never expands the selection to satisfy this). Every submitted ID and every selected Lesson's content/Quiz aggregate is revalidated from a fresh in-transaction read using the exact same `evaluateLessonContentBlockers`/`evaluateQuizPublishability` evaluators the Course Readiness slice introduced — never a second, divergent publishability definition — and any single stale/invalid item rejects the entire selection atomically (`PUBLISH_SELECTION_STALE`, 409, carrying the current blockers in the existing `ReadinessIssue` shape as an additional field on the standard error envelope, not a new top-level HTTP shape). A selected QUIZ Lesson's Quiz ID is always resolved from its own live `quizLesson` relation; every distinct required Quiz is locked via the existing `lockQuizPublicationBoundary`, in sorted `quizId` order, before any Quiz aggregate is read, matching the established multi-Quiz lock-ordering convention.
- Reasoning: Requiring Sections/Lessons to already be individually `PUBLISHED` before they could be included in a Course's first publish (the readiness slice's original, corrected draft — see DEC-0049) would force the Instructor through the exact multi-click "publish every Section, then every Lesson" workflow this redesign exists to remove. Explicit selection lets the Instructor review the server-computed `readyToPublish` candidate set once and publish exactly that reviewed set in one atomic action, while still never trusting the client's prior readiness response, titles, or types — everything is re-proven from current database state inside the mutating transaction.
- Implications: The Course's first-publish transition is claimed with one conditional `UPDATE ... WHERE status = DRAFT AND published_at IS NULL`, checked by affected-row count, as the very first write in the transaction — before any Section/Lesson/Quiz is read — so the Postgres row lock it takes serializes against any concurrent publish-selected/`/publish`/`/archive`/`/unpublish`/`/restore` call on the same Course, and a lost race is detected before any descendant work happens rather than discovered only after Sections/Lessons/Quizzes have already been mutated. No new advisory lock was introduced for Course/Section/Lesson — the existing conditional-update-plus-affected-row-count pattern already used by every lifecycle method is sufficient there; Sections/Lessons have no lock of their own, and a narrow concurrent-mutation window is closed defensively by re-checking affected-row counts and re-deriving accurate blockers from a fresh read rather than silently reporting success. Every query is bounded by the submitted IDs or Quiz IDs server-derived from the submitted Lessons' own relations — never a course-wide "find every Draft/ready row" sweep — so an unselected Section/Lesson/Quiz is never read, let alone published, regardless of how ready it independently is. `Course.publishedAt` is confirmed, by reading every Course-mutating method, to be set exactly once and never cleared by Take Offline or Restore, making `publishedAt !== null` a permanently safe "published before" signal even though a republish through the granular `/publish` endpoint does overwrite it with a fresh timestamp (pre-existing behavior, unchanged). No schema or migration change was required.
