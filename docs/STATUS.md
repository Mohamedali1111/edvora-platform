...
- Student Document Lesson access (`GET /student/courses/:courseId/lessons/:lessonId/document/access`) reuses the same `StudentCourseAccessService` entitlement chain via a new `assertAccessibleDocumentLesson` method (mirroring `assertAccessibleQuizLesson`'s pattern), additionally requiring the linked `DocumentAsset.processingStatus` to be `READY`. `MediaModule` now imports `CoursesModule` and `DeviceModule` one-directionally, mirroring `QuizzesModule`'s existing import shape; `CoursesModule` imports neither. This is a provider-independent authorization boundary only — it issues no signed URL/download token, since no storage/video provider has been selected yet.
- Student Video Lesson playback authorization (`GET /student/courses/:courseId/lessons/:lessonId/video/access`) reuses the exact same `StudentCourseAccessService` entitlement chain via a new `assertAccessibleVideoLesson` method, mirroring `assertAccessibleDocumentLesson`'s pattern exactly, additionally requiring the linked `VideoAsset.processingStatus` to be `READY`. It is registered in the already-existing `MediaModule` (no further module-import changes needed). This is a provider-independent authorization boundary only — it issues no playback URL/token/DRM material, since no video/streaming provider has been selected yet.
...
- Media Slice A exists for instructor-side VideoAsset/DocumentAsset tenant-scoped reads only. Media Slice B exists for the student-side DOCUMENT Lesson runtime *authorization* boundary only (`GET /student/courses/:courseId/lessons/:lessonId/document/access`) — it proves entitlement and READY-state document readiness and returns only safe metadata, never a download capability. Media Slice C exists for the equivalent student-side VIDEO Lesson runtime *authorization* boundary only (`GET /student/courses/:courseId/lessons/:lessonId/video/access`) — it proves entitlement and READY-state video readiness and returns only safe metadata (`durationSeconds`, `ready`, `authorizedAt`), never a playback URL/token/DRM material. Asset creation/registration, upload intents, provider callbacks, actual signed/ephemeral document/video access issuance, real video streaming, and DRM integration remain deferred until a real storage/video provider is selected.
- Course authoring Slice A (core course metadata), Slice B (Section/Lesson create, metadata update, archive, and whole-list reorder), Slice C (student course authorization/read), and Slice D (minimal lesson progress) exist. Lesson creation for VIDEO/DOCUMENT/QUIZ types requires the instructor to reference an already-existing tenant-scoped VideoAsset/DocumentAsset/Quiz row by ID; Quiz authoring now exists (see below), but VideoAsset/DocumentAsset creation still has no upload flow, so real end-to-end lesson creation for those two types remains gated on that future work landing. A currently-unavailable lesson (before `availableFrom` or at/after `availableUntil`) is omitted entirely from the student structure response rather than returned as locked metadata — the conservative choice since neither `docs/PRODUCT.md` nor `docs/BACKEND-DOMAIN.md` specify this, and omission leaks no title/type/existence about content the student cannot yet reach. Lesson progress is currently limited to a tri-state status (NOT_STARTED/STARTED/COMPLETED), with manual completion for non-quiz lessons and automatic completion for QUIZ lessons on a qualifying Quiz Attempt (Quiz Milestone Slice D — see below) — video watch-time/resume-position writes and persisted aggregate course percentages are deliberately not implemented. Course lifecycle transitions, actual signed/ephemeral document/video access issuance, real video streaming, and frontend/mobile course UI remain pending (the DOCUMENT and VIDEO Lesson runtime *authorization* boundaries themselves exist — see Media Slice B and Slice C below).
...
- No formal provider-port interface was introduced (see `docs/MEDIA.md`); no HTTP route resembling `/student/documents/:documentAssetId` was added; no signed URL, download token, or provider credential was fabricated anywhere in the response or tests.
