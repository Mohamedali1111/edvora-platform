...
- Instructor Quiz Authoring (Quiz/Question/QuestionOption create/read/update/reorder), Quiz Milestone Slice B (student-safe Quiz content delivery, `GET /student/courses/:courseId/lessons/:lessonId/quiz`), and Quiz Milestone Slice C (student Quiz attempt start/read/answer/submit with server-side scoring, `attemptLimit` enforcement, and an immutable per-attempt passing-threshold snapshot) exist. Quiz-derived `LessonProgress` completion (a QUIZ Lesson still cannot be completed by any means — manual completion is deliberately rejected by Slice D, and attempt-based completion is intentionally out of scope for Slice C) and attempt result/review UI remain pending; that is the next quiz-domain gap. `clientSubmissionKey`-based idempotent attempt start (an optional schema field for deduplicating mobile retry-created attempts) was also not implemented, as it was not required by Slice C's specified request shape; submission itself is fully idempotent via attempt-status short-circuiting, satisfying `docs/BACKEND-DOMAIN.md`'s "idempotent with a client request key or attempt state transition" either-or requirement.
...
Design quiz-derived `LessonProgress` completion (deriving lesson completion from a `GRADED` Quiz Attempt) and an attempt result/review screen on top of the now-complete Slice C attempt engine, and the protected video/document access boundary (playback/download authorization) per `docs/DECISIONS.md`'s DRM-ready-not-DRM-implemented requirement — these are the remaining major pieces of the course-content domain. Also run a full auth/device/tenancy/workspace validation gate, since none has been run since the Course Slice B review's shared-utility fix.
