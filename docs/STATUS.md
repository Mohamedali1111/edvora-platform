...
- Platform Admin instructor onboarding, instructor tenant/student/enrollment APIs, instructor course metadata APIs, instructor course section/lesson authoring APIs, student course authorization/read APIs, and student lesson-progress read/completion APIs exist. Activation delivery, student removal endpoints, course lifecycle transitions, protected content delivery (video/document/quiz), video watch-time/resume tracking, quiz-derived progress, and distributed rate limiting are not implemented.
...
Design the protected video/document access and quiz-execution boundaries as separate, later endpoints per `docs/DECISIONS.md`'s DRM-ready-not-DRM-implemented requirement — these are the last major pieces of the course-content domain, and both can now build on the same `StudentCourseAccessService` entitlement chain Slices C and D established. Before implementing either, run a full auth/device/tenancy/workspace validation gate, since none has been run since the Slice B review's shared-utility fix.
