...
- Media Slice A exists for instructor-side VideoAsset/DocumentAsset tenant-scoped reads. Media Slice B exists for the student-side DOCUMENT Lesson runtime authorization boundary (`GET /student/courses/:courseId/lessons/:lessonId/document/access`) and now issues an ephemeral R2 download capability for the finalized READY object after that boundary succeeds. Media Slice C exists for the equivalent student-side VIDEO Lesson runtime authorization boundary (`GET /student/courses/:courseId/lessons/:lessonId/video/access`) — it proves entitlement and READY-state video readiness and returns only safe metadata (`durationSeconds`, `ready`, `authorizedAt`), never a playback URL/token/DRM material. R2 document upload/download capability issuance exists; real video streaming, Bunny integration, DRM, piracy prevention, abandoned-upload cleanup, and provider-object cleanup remain deferred.
...
- Provider selection is locked for V1 media: Documents = Cloudflare R2; Videos = Bunny Stream
  Standard Network. Remaining media decisions are Bunny playback/DRM integration details and future
  cleanup/operational policy, not provider reselection.
...
has been run since the Course Slice B review's shared-utility fix.
